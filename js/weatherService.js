...
const WeatherService = (() => {
  ...
  /* ──────────────────────────────────────────
     PUBLIC: Get Weather by City Name
     ────────────────────────────────────────── */
  ...
  async function searchCities(query) {
    ...
    // Instant match from local database (60+ Indian cities)
    const localMatches = MockData.searchCities(query);
    if (localMatches.length > 0) {
      return localMatches;
    }

    try {
      const url = `${CONFIG.openMeteoGeoUrl}?name=${encodeURIComponent(query)}&count=10&language=en&format=json`;
